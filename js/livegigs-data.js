...
(function() {
    ...
    // 安全设置文本内容
    function safeSetText(element, value, defaultValue = '') {
        if (!element) return;
        if (!isEmpty(value)) {
    ...
    // 安全设置HTML内容
    function safeSetHTML(element, value, defaultValue = '') {
        if (!element) return;
        if (!isEmpty(value)) {
    ...
    // 安全设置链接
    function safeSetLink(element, url, text, target = '_blank') {
        if (!element) return;
        if (!isEmpty(url)) {
            element.href = url;
    ...
    // 安全设置图片
    function safeSetImage(element, src, alt = '') {
        if (!element) return;
        if (!isEmpty(src)) {
            element.src = src;
            element.alt = alt;
    ...
    function renderBanners(data, pageType) {
        ...
        if (pageType === 'index' || pageType === 'cn') {
            container = document.querySelector('.banner-slider, #bannerSlider, .hero-slider');
        }
    ...
    function renderPosters(data, pageType) {
        ...
        posters.forEach((poster, index) => {
            ...
            safeSetImage(img, poster.image, poster.title || '');
    ...
    function renderFooter(data, pageType) {
        ...
        // 站点名称
        const siteNameEl = document.querySelector('.footer-brand, .site-name, #footer-brand');
        if (siteNameEl) {
            ...
            siteNameEl.innerHTML = `${nameText}<span style="color: #8b0000;">${asiaText}</span>`;
        }

        // 联系文字
        const contactTextEl = document.querySelector('.footer-contact-text, .contact-text');
        if (contactTextEl && !isEmpty(data.contactText)) {
            contactTextEl.textContent = data.contactText;
        }

        // 邮箱 - 可点击发送邮件
        const emailEl = document.querySelector('.footer-email, .email-link');
        if (emailEl && !isEmpty(data.email)) {
        ...
        // 地址
        const addressEl = document.querySelector('.footer-address, .address-text');
        if (addressEl && !isEmpty(data.address)) {
            addressEl.textContent = data.address;
        }

        // 版权文字 - 修复版
        const copyrightEl = document.querySelector('.copyright, .footer-copyright, #footer-copyright');
        if (copyrightEl) {
        ...
        // 制作单位Logo
        const producerEl = document.querySelector('.producer-logo, .footer-producer, #footer-producer');
        if (producerEl) {
            if (!isEmpty(data.producerLogo)) {
                producerEl.src = data.producerLogo;
                producerEl.style.display = '';
            } else {
        ...
        // 社交媒体 - 修复拉伸问题
        const socialContainer = document.querySelector('#socialContainer, .footer-social, .social-links');
        if (socialContainer && data.socials) {
            const enabledSocials = data.socials.filter(s => s.enabled && !isEmpty(s.icon));
            ...
            enabledSocials.forEach(social => {
                ...
                // 创建图标 - 使用SVG防止拉伸
                const iconSvg = getSocialIconSvg(social.icon);
                ...
                link.style.backgroundColor = '#050505';
    ...
    async function loadAndRender() {
        ...
        switch(pageType) {
            ...
            case 'events':
                ...
                renderPosters(indexPostersData, 'events'); // Events页面也有3个海报
