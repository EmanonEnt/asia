...
(function() {
    ...
    function renderFooter(data) {
        ...
        console.log('[LiveGigs] Rendering footer with data:', data);

        // 1. 更新版权信息 - 适配 .copyright 类
        const copyrightElements = document.querySelectorAll('.copyright, #footer-copyright, [data-editable*="copyright"]');
        ...
        socialContainers.forEach(container => {
            ...
            console.log('[LiveGigs] Updating social icons:', data.social.length);
            ...
            data.social.forEach((item, index) => {
                ...
                a.target = '_blank';
                a.setAttribute('data-editable', `social-${item.platform}`);
                a.setAttribute('data-href', item.link || '#');
                a.setAttribute('data-platform', item.platform);
                a.title = item.name || item.platform;
                ...
                const svgPaths = {
                    ...
                    'xiaohongshu': 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-2h2v2zm0-4h-2V7h2v6zm4 4h-2v-2h2v2zm0-4h-2V7h2v6z',
                    ...
                    'miniprogram': 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-2h2v2zm0-4h-2V7h2v6zm4 4h-2v-2h2v2zm0-4h-2V7h2v6z'
                };

                const path = svgPaths[item.platform] || svgPaths['facebook'];
                a.innerHTML = `<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="${path}"/></svg>`;

                container.appendChild(a);
                console.log('[LiveGigs] Added social icon:', item.platform);
            });
        });

        // 3. 更新Producer logo
        const producerElements = document.querySelectorAll('.producer-logo, #footer-producer img, [data-editable="producer-logo"]');
        producerElements.forEach(el => {
            if (data.producer) {
                el.src = data.producer;
                el.setAttribute('data-src', data.producer);
                console.log('[LiveGigs] Updated producer logo:', data.producer);
            }
        });

        // 4. 更新联系信息
        if (data.email) {
            const emailElements = document.querySelectorAll('.contact-item a[href^="mailto"], [data-editable="footer-email"] a');
        ...
        if (data.phone) {
            const phoneElements = document.querySelectorAll('.contact-item a[href^="tel"], [data-editable="footer-phone"] a');
        ...
        console.log('[LiveGigs] Footer rendered successfully');
    }

    // 主函数
    async function init() {
        const pageType = getPageType();
        console.log('[LiveGigs] Initializing for page:', pageType);

        // 确定加载哪个footer配置
        const footerFile = pageType === 'cn' ? 'footer-cn' : 'footer-global';

        const data = await loadJSON(footerFile);
        if (data) {
            renderFooter(data);
        }
    }
    ...
    console.log('[LiveGigs] Data loader initialized');
})();
